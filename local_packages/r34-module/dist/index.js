...
__export(index_exports, {
  hypno_random: () => hypno_random,
  hypno_search: () => hypno_search,
  r34_random: () => r34_random,
  r34_search: () => r34_search,
  real_random: () => real_random,
  real_search: () => real_search,
  safe_random: () => safe_random,
  safe_search: () => safe_search,
  xbooru_random: () => xbooru_random,
...
const RULE34_API_KEY = 'd1f5e10296437cdab852d5b7565020b508062f0bd6277af1c6370e6166394e168f8b5851826b96f87ce7e4ce0743a7e127284b706f309a217f2f1c2b68ad08c0';
const RULE34_USER_ID = '5278018';
const AUTH_PARAMS = `&api_key=${RULE34_API_KEY}&user_id=${RULE34_USER_ID}`;
...
async function r34_random({ gay_block } = { gay_block: false }) {
  ...
  let { data } = await (0, import_axios2.default)(
    `https://api.rule34.xxx/index.php?page=dapi&s=post&pid=${random}&q=index&json=1${AUTH_PARAMS}`
  ); 
  
  if (!Array.isArray(data)) {
      console.error("Rule34 API response was not an array (r34_random):", data);
      return [];
  }
...
async function r34_search({ search_tags = [], block_tags = [] }) {
  ...
  if (pages != 0) {
    let random = Math.floor(Math.random() * pages);
    let { data } = await (0, import_axios2.default)(
  `https://api.rule34.xxx/index.php?page=dapi&s=post&tags=${search_tag}&pid=${random}&q=index&json=1${AUTH_PARAMS}`
);
    let ray = [...data.map((obj) => obj.file_url)];
...
var hypno_random = async ({ gay_block } = { gay_block: false }) => {
  ...
  let random = Math.floor(Math.random() * pages);
  let { data } = await (0, import_axios4.default)(
...
var safe_random = async () => {
  ...
  let random = Math.floor(Math.random() * pages);
  let { data } = await (0, import_axios5.default)(
