...
const { arcloud } = require('./ARQUIVES/js/arcc.js');
const { addBanned, unBanned, BannedExpired, cekBannedUser } = require("./ARQUIVES/js/banned.js");
    const namoro1 = JSON.parse(fs.readFileSync("./corvo_dados/func/namoro1.json"));
const namoro2 = JSON.parse(fs.readFileSync("./corvo_dados/func/namoro2.json"));
...
const { wait, getExtension, generateMessageID, getMembros, getGroupAdmins, getRandom, banner, banner2, banner3, banner4, temporizador, chyt, getBuffer, fetchJson, fetchText, createExif, getBase64, convertSticker, upload, nit, getpc, supre, recognize } = require('./ARQUIVES/funcoes/functions.js'); // É necessário para algumas funções funcionar perfeitamente.
...
// [JSON] - Funções Abaixo >>> :
const pushnames = JSON.parse(fs.readFileSync('./corvo_dados/usuarios/users.json'));
const vdddsf = JSON.parse(fs.readFileSync("./corvo_dados/vdddsf.json"));
const obrigadoEXT = JSON.parse(fs.readFileSync('./INFON/media/nescessario.json'))
const rgtake = JSON.parse(fs.readFileSync('./corvo_dados/take.json'));
const sotoy = JSON.parse(fs.readFileSync('./ARQUIVES/json/sotoy.json'));
const countMessage = JSON.parse(fs.readFileSync('./INFON/media/countmsg.json'));
const comandos = JSON.parse(fs.readFileSync('./INFON/media/comandos.json'));
const daily = JSON.parse(fs.readFileSync('./corvo_dados/usuarios/diario.json'));
const nescessario = JSON.parse(fs.readFileSync('./INFON/media/nescessario.json'));
const ban = JSON.parse(fs.readFileSync('./corvo_dados/usuarios/banned.json'));
const muted = JSON.parse(fs.readFileSync('./corvo_dados/grupos/muted.json'))
const limitefll = JSON.parse(fs.readFileSync('./corvo_dados/usuarios/flood.json'));
const joguinhodavelhajs = JSON.parse(fs.readFileSync('./corvo_dados/usuarios/joguinhodavelha.json'));
const joguinhodavelhajs2 = JSON.parse(fs.readFileSync('./corvo_dados/usuarios/joguinhodavelha2.json'));
const antispam = JSON.parse(fs.readFileSync('./INFON/media/antispam.json'));
const anotar = JSON.parse(fs.readFileSync("./corvo_dados/func/anotar.json"));
const setting = JSON.parse(fs.readFileSync('./INFON/DADOS/config.json'));
const logoslink = JSON.parse(fs.readFileSync('./INFON/LOGOS/links_img.json'));
const Limit_CMD = JSON.parse(fs.readFileSync("./corvo_dados/func/limitarcmd.json"))
const ftmenu = JSON.parse(fs.readFileSync("./INFON/LOGOS/logos.json"));
...
const { linguagem, mess, getInfo } = require('./INFON/lib');
...
var EnvBotao = async (id, MR, tokito, text1, desc1, X) => {
if(X.split("|")[1] != "0") {
tokito.sendMessage(id, {image: {url: X.split("|")[1]}, caption: text1, mentions: [MR]})} else {tokito.sendMessage(id, {text: text1, mentions: [MR]})}}
} else {
var EnvBotao = async(id, MR, tokito, text1, desc1, X, TXT = [], vr) => {
...
tokito.sendMessage(id, buttonMessage, {quoted: vr}).catch(e => {
...
const sendPoll = (nagatoro, id, name = '', values = [], selectableCount = 1, mentionedJid = []) => { 
return nagatoro.sendMessage(id, {poll: {name, values, selectableCount}, mentions: mentionedJid, contextInfo: { mentionedJid, messageSecret: randomBytes(32)}}, {id, options: {userJid: nagatoro?.user?.id}}).catch(() => {
...
// 🐛 FIX 2026-08-10: jid @lid (LID) gerava "+li d -d" porque identArroba pegava
// o DOMÍNIO ("lid") em vez do número. Agora extrai a parte numérica antes do @.
let usu = String(nmr || '');
usu = usu.includes('@') ? usu.split('@')[0] : identArroba(usu).split('@')[0];
// 🛡️ Remove TUDO que não for dígito (cobre sufixo de device ":3", espaços, +, -)
usu = usu.replace(/\D/g, '');
if (!usu) return "+?";
if (usu.length < 8) return "+" + usu;
return "+" + usu.slice(0, 2) + " " + usu.slice(2, 4) + " " + usu.slice(4, usu.length - 4) + "-" + usu.slice(usu.length - 4, usu.length);
}

const identArroba = (txt) => {
// 🐛 FIX: jid completo (com @) → pega a parte ANTES do @ (o número), não o domínio.
const numero = txt.includes('@') ? txt.split('@')[0] : txt;
return numero.replace(new RegExp("[()+-/ +/]", "gi"), "") + "@s.whatsapp.net";
}
...
module.exports = { extractDDD, extractStateFromNumber, path, extractStateFromDDD, formatDateOriginal, awaitMessage, LoggerB, readline, P, fs, util, Boom, axios, linkfy, request, ms, ffmpeg, fetch, qrterminal, exec, extractMetadata, spawn, execSync, limitefll, moment, time, hora, date, os, getBuffer, convertSticker, fetchJson, fetchText, getBase64, createExif, writeExifImg, upload, nit, addBanned, unBanned, BannedExpired, cekBannedUser, validmove, setGame, addComandosId, deleteComandos, getComandoBlock, getComandos, addComandos, palavrasANA, quizanimais, getpc, supre, wait, getExtension, generateMessageID, getGroupAdmins, getMembros, getRandom, banner, banner2, banner3, temporizador, chyt, simih, antispam, anotar, sotoy, countMessage, comandos, daily, muted, nescessario, ban, joguinhodavelhajs, joguinhodavelhajs2, setting, logoslink, ftmenu, linguagem, getInfo, mess, destrava, destrava2, tabela, recognize, colors, cheerio, NodeCache, kyun, TimeCount, sendVideoAsSticker, sendImageAsSticker, sendVideoAsSticker2,sendImageAsSticker2, enviarfiguUrl, sendPoll, getFileBuffer, DLT_FL, sleep, ANT_LTR_MD_EMJ, convertBytes, arcloud, os, garticArchives, enigmaArchive, isFiltered, addFilter, psycatgames, speed, vyroEngine, AssemblyAI, whatMusicAr, quizFutebol, obeso, countDays, timeDate, writeExif2, Limit_CMD, capitalizeFirstLetter, emoji, formatNumber, formatNumberDecimal, isJsonIncludes, pushnames, shuffle, packname, advices, tools, getName, listCommands, namoro1, namoro2, obrigadoEXT, addNumberMais, identArroba, banner4, getBotKnowledge }